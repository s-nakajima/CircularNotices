<?php
/**
 * CircularNoticeSettingsController Controller
 *
 ...
 */
class CircularNoticeSettingsController extends CircularNoticesAppController {
 ...
 */
	public function edit() {
		...
		// タブの設定
		$this->initSettingTabs('circular_notice_settings');

		$permissions = $this->NetCommonsBlock->getBlockRolePermissions($this->viewVars['blockKey'], ['content_creatable']);
		...
		if ($this->request->is('post')) {
			...
			if ($this->handleValidationError($this->CircularNoticeSetting->validationErrors)) {
				if (! $this->request->is('ajax')) {
					$this->redirect('/circular_notices/blocks/index/' . $this->viewVars['frameId']);
				}
				return;
			}
		}
